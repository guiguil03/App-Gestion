...
export function buildCardHtml({
  ...
  const cardNumber = cardId.replace(/-/g, '').slice(0, 8).toUpperCase();
  const formattedBirth = formatDate(dateOfBirth);
  const formattedIssued = formatDate(issuedAt);
    ...
    <html>
      ...
      <body>
        <div class="card">
          ...
          <div class="body">
            ${photoUrl ? `<img class="photo" src="${photoUrl}" />` : `<div class="initial">${initial}</div>`}
            <div class="info">
              <p class="name">${fullName}</p>
              <span class="class-pill">${className} · ${promotion}</span>
              <div class="grid">
                <div>
                ...
                <div>
                  ...
                  <p class="value">${sex === 'F' ? 'Féminin' : 'Masculin'}</p>
                </div>
              </div>
            </div>
            <div class="qr-block">
